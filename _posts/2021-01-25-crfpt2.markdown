...
To learn a model that can annotate examples like the one above with POS tags, we need to extract proper
features from the input sequence, which we will do with a bidirectional LSTM (motivated below). 
Then what we'll implement below is:
...
can be interpreted as transition likelihood from one tag to another. The parameters $$\boldsymbol{\theta}_2$$ are shared
...
Secondly, we need to talk about broadcasting. Broadcasting over dimensions is conceptually the same thing as matrix multiplication of a column vector with a row vector,
...
We discussed above that $$\boldsymbol{\theta}_1f(y^{\prime}_{t+1}, \mathbf{x}, t+1) = \mathbf{H}_{t+1,y_{t+1}}$$ is the t+1-th row and $$y_{t+1}$$-th column of the projected output of our encoder. These values are collected in a vector called `unary_features` for all $$y^{\prime}_{t+1}$$ (meaning the vector has size $$|S|$$). In the above
 recursion, these values are the same for all $$y_t^{\prime}$$ in the sum. Then $$\boldsymbol{\theta}_2f(y^{\prime}_t, y^{\prime}_{t+1}) = \mathbf{T}_{y_t,y_{t+1}}$$
...
To finish calculating the loss, now that we calculated the partition function, we just need to calculate the (log-)nominator!

$$\sum_{t=1}^m \boldsymbol{\theta}_1 f(y_t^{(i)}, \mathbf{x}^{(i)}, t) + \sum_{t=1}^{m-1} \boldsymbol{\theta}_2 f(y_t^{(i)}, y_{t+1}^{(i)})$$
...
and target tag sequence $$\mathbf{y}^{(i)}$$. In the implementation we loop over all the sequences in the batch,
...
Below I took out a piece of code that's inside if the `if`-statements in the code above. What happens is that we calculate:
...
(depending on how long each sequence in the batch is the actual batch might look differently).
...
Go to [part three]({% post_url 2021-01-25-crfpt3 %}) to train this model on real POS tagging data!
