...
If you want to skip the background and start implementing, go to [part 2]({% post_url 2021-01-25-part2 %}), where we'll put a linear-chain CRF on top of a bidirectional
...
$$\mathbf{y}$$ (we will clarify what this function is later), then prediction can be denoted by the following equation:
...
&= \sum_{t=1}^m\sum_{y_1^{\prime}}\dots\sum_{y_m^{\prime}} p(\mathbf{y}^{\prime} \mid \mathbf{x})  \cdot f(y_t^{\prime},\mathbf{x},t) \\
&= \sum_{t=1}^m\sum_{y^{\prime}_t} p(y^{\prime}_t \mid \mathbf{x})  \cdot f(y_t^{\prime},\mathbf{x},t)
\end{aligned}
$$
...
$$
\begin{aligned}
\nabla_{\boldsymbol{\theta}_2} \log \mathcal{L}(\boldsymbol{\theta}) &= \sum_{t=1}^m\sum_{y^{\prime}_t}\sum_{y^{\prime}_{t+1}} p(y^{\prime}_t,y^{\prime}_{t+1} \mid \mathbf{x})  \cdot f(y_t^{\prime},y_{t+1}^{\prime})
\end{aligned}
$$
...
computing the partition function down to polynomial time. Additionally, to calculate the gradients of the log-likelihood w.r.t. the parameters, 
 ...
 Before I knew dynamic programming the term always sounded difficult to me, but it really is just nothing other than re-using
 stuff you already computed before. E.g., if you have a function that calculates $$n!$$ and it's called more often
 than once you can store the computations and re-use them when the function is called again.
 How BP works can be illustrated well when we write out the sums and products in the partition function 
...
In the below animation we can see the computations done in the forward- and backward-pass of Viterbi. We initialize the leftmost
...
(using $$\bar{p}(\cdot)$$ here because to distinguish it from the marginal probability of $$y_1$$ given $$\mathbf{x}$$).
As the algorithm continues backpointers can take different paths. At the final column we can start actually decoding and decide what the maximizing final POS-tag is: $$\hat{y}^{\prime}_m = arg\max_{y^{\prime}_m}\psi(y^{\prime}_m) \cdot v_{m-1}(y^{\prime}_m)$$.
...
like LSTM's, and train everything end-to-end with stochastic gradient ascent on the likelihood! All part of [part 2]({% post_url 2021-01-25-part2 %}) of this series.
