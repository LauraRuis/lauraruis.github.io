...
*Side notes*. There is a lot more to unpack in this image, much of which we did not get into in the main paper, if only because it's hard to draw conclusions. For example, we cannot say why InstructGPT-3 does better than
...
Perhaps we can improve the zero-shot performance of the best models with better prompting? For the three OpenAI models (GPT-3, InstructGPT-3-175B, and text-davinci-002) we attempt the zero-shot experiment again, but now with more elaborate prompts containing detailed instructions (see example introduced above). It does not help.
