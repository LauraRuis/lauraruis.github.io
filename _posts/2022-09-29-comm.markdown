...
In <a href="TODO: arxiv link" target="_blank">our most recent paper</a>'s title we state that LLMs are not zero-shot communicators. How can these two things both be true? What do we mean by "communicators"? In this post I'll motivate the statement by summarising the paper's results. 

Consider the following exchange between a human and InstructGPT.

> Human: Have you seen my keys? \\
> InstructGPT: Yes, I have seen your keys.
...
the location of a set of keys.
Humans use commonsense knowledge like an experience of having lost their own keys in the past to infer that the speaker is probably looking for them instead of actually wanting a yes-no response.
...
To see a more in-depth introduction of implicature refer to Appendix B of <a href="TODO: arxiv link" target="_blank">our paper</a>.
...
For all the results, check out <a href="TODO: arxiv link" target="_blank">the paper</a>. Here I'll briefly summarise the main findings.
...
GPT-2, GPT-3, OPT, BLOOM, and EleutherAI, (2) instructable LLMs finetuned with reinforcement learning (OpenAI's Davinci-001 and Davinci-002 of the class InstructGPT),
(3) instructable LLMs finetuned on downstream tasks (T0 and Flan-T5), and (4) LLMs finetuned on dialogue (BlenderBot).
...
OpenAI's instructable models are the best model classes we look at, with InstructGPT obtaining 72% zero-shot accuracy. This leaves a gap of 13.9% with human performance. The best models of the other classes we look at achieve between 53.4% (BlenderBot-2.7B) and  63.3% (Flan-T5-780M) zero-shot accuracy.
...
The instructable models InstructGPT and Davinci-002 obtain a significantly better performance than all other models,
respectively obtaining 72% and 70.6% accuracy. This is 7.3% better than the best model of all other groups. Perhaps we can improve their zero-shot performance with better prompting?

_For details see the paper: section 4.1, Figure 2 left, Table 1_

#### <span style="color:#C0392B">Instruction prompts do not help</span>
For the three OpenAI models (GPT-3, InstructGPT, and Davinci-002) we attempt the zero-shot experiment again, but now with more elaborate prompts containing detailed instructions. This does not improve performance.

_For details see the paper: section 4.1, Table 3_

#### <span style="color:#C0392B">In-context prompting helps a lot</span>
We use the original 6 prompt templates, but now add in-context examples. At $$k=30$$ we find that performance jumps to 80.6% for Davinci-002, which is near-human performance! It does not help much for the other models.
...
is 8.8% for the best performing model (Davinci-002).
...
the prospects of large language models becoming "communicators" by improving their pragmatic language skills. You can read about our work in more detail <a href="TODO: arxiv link" target="_blank">here</a>.
