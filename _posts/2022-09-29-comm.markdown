...
<u>Communication is</u> a collaborative effort where speakers try to convey meaning to each other. A "communicator" can successfully
...
OpenAI's instructable models are the best model classes we look at, with Davinci-001 obtaining 72% zero-shot accuracy. This leaves a gap of 13.9% with human performance. The best models of the other classes we look at achieve between 53.4% (BlenderBot) and 61.5% (OPT) zero-shot accuracy.
...
The instructable models Davinci-001 and Davinci-002 obtain a significantly better performance than all other models,
respectively obtaining 72% and 70.6% accuracy. This is 8.5% better than the best non-intructable model. Perhaps we can improve their zero-shot performance with better prompting?

_For details see the paper: section 4.1, Figure 2 left, Table 1_

#### <span style="color:#C0392B">Instruction prompts do not help</span>
For the three OpenAI models (GPT-3, Davinci-001, and Davinci-002) we attempt the zero-shot experiment again, but now with more elaborate prompts containing detailed instructions. This does not improve performance.

_For details see the paper: section 4.1, Table 3_

#### <span style="color:#C0392B">In-context prompting helps a lot</span>
We use the original 6 prompt templates, but now add in-context examples. At $$k=30$$ we find that performance jumps to 80.6% for Davinci-002, which is near-human performance! It does not help much for the non-instructable models.
